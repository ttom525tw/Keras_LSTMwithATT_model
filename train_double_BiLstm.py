...
def train_double_BiLstm(p_n_symbols, p_embedding_weights, p_X_train, p_y_train, p_X_test, 
    ...
    model_t.add(Bidirectional(LSTM(output_dim=50)))
    ...
    plt.legend(['train', 'test'], loc='best')
    plt.show()
    plt.savefig('double_BiLstm')
    #儲存模型
    model.save('double_BiLstm.h5')
